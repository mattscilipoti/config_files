...
#TODO: how do I use a local var for this?
local RVM_PROMPT="%{$fg[blue]%}$(~/.rvm/bin/rvm-prompt i g)%{$reset_color%}"

PROMPT='%{$fg[green]%}%1~%{$reset_color%} ${RVM_PROMPT} $(git_prompt_info)%{$reset_color%}%B$%b '
# RPROMPT="${return_code}"
# RPROMPT="${RVM_PROMPT}${return_code}"
#TODO: rvm was NOT working with local var, needed direct entry.
RPROMPT='${return_code}%{$fg[blue]%}$(~/.rvm/bin/rvm-prompt i g)%{$reset_color%}'
